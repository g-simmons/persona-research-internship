DualAlpha := module()
option package;
export pdiag,randpd,getalpha,drawpd,pcell,pgraph,pcech,witmap,vectab,drawalpha;
...
#be consistent with existing notation
    getalpha := proc(pd,dim)
        if(type(args[1],'Matrix')) then
            # what is power vector?
            if(not type(args[2],'Vector')) then
                A,r := args[1..2];
                N := Dimension(A)[1];
                if(r<.1 or r>10.0) then
                    error "since active set methods use tolerance variables, it's better to rescale so that the radius is around 1.0";
                end if;
                return getalpha(pdiag(A,allocla[float[8]](N),r^2),args[3]);
            else
                return getalpha(pdiag(args[1..3]),args[4]);
            end if;
        end if;
        cech := pd:-cech;
        printf("computing cech graph G...\n");
        X := cech:-getnerve();
...
#representative
    witmap := proc(X,m)
        md := module()
        option object;
        export X,m,A,setelt,getvec,getelt,init,flags;
        local dyn,ModulePrint,ModuleApply;
            ModulePrint::static := proc()
...
# L1.G is multiplication of L1 by some matrix G
#Lt=L gives the usual metric.
    drawsv := proc(L,pow,il,rng)
    uses LinearAlgebra;
    local x,y;
        L1,L2 := metpair[op(getsubs(procname))](L);
        # if G is the identity matrix, these two should be the same
        n,m := Dimension(L1);
        if(m<>2) then
    ...
    li2poly := proc(A,V,N,P)
        ...
        pl := [seq([
            P[i,1],P[i,2]],i=1..n)]; 
        print(pl); # first two rows of pl are duplicated, causing zero values in ql and rl
        ql := [seq(pl[i]-pl[1],i=2..n)];
        # rl := [seq(sqrt(q[1]^2+q[2]^2),q=ql)];
        rl := [seq(sqrt(q[1]^2+q[2]^2 + .001),q=ql)];
        # containsZero := false;

        # for element in rl do
        #     if element = 0 then
        #         containsZero := true;
        #         error "Zero value found in rl";
        #         break;
        #     end if;
        # end do;
        c0 := 1;
        ...
        # chatgpt suggestion
        # if n > 1 then
        #     c0 := (ql[1][1]*ql[1][1]+ql[1][2]*ql[1][2])/(rl[1]*rl[1]); # Assuming ql[1] is valid here and corresponds to pl[2]-pl[1]
        #     i0 := 1; # Since ql starts from the second element, this should actually correspond to the second element.
        # else
        #     c0 := 1;
        #     i0 := 0;
        # end if;
        ...
        cl := [seq((ql[i0][1]*ql[i][1]+ql[i0][2]*ql[i][2])/rl[i0]/rl[i],i=1..n-1)]; # getting an undefined value here
        #chatgpt suggestion 
        # cl := [seq((ql[i0][1]*ql[i][1]+ql[i0][2]*ql[i][2])/rl[i0]/rl[i],i=2..n)]; # start from 2
        if not (type(cl, 'list') and (type(cl[1], 'numeric') or type(cl[1], 'string'))) then
            print(rl);
            print(cl);
            error "cl must be a numeric or string list";
        end if;
        # sig := sort(cl,`>`,output=permutation);
        sig := sort(cl[2..],`>`,output=permutation);
        # return [pl[1],seq(pl[sig[i]+1],i=1..n-1)];
        print(sig);
        print(pl);
        return [seq(pl[sig[i]+1],i=1..n-2)];
    end proc;

    metpair := proc(A)
        if(type(A,'list')) then
            A1 := A[1]; # two matrices already provided, drop 2nd
        else
            A1 := A; #first arg is a matrix
        end if;
        A2 := A1; # copy A2 to A1
        # In the context of the metpair procedure you provided, 
        # op(procname) seems to be extracting information about the calling procedure, 
        # which is a bit unconventional and would typically be part of a complex or 
        # reflective programming pattern where the function's behavior depends on how it was called.
        if(type(procname,indexed) and nops(procname)=1) then
            G := op(procname); # op stands for operands
            A2 := A2.G;
        end if;
