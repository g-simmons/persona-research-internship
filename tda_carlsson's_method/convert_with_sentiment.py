...
jsonl_file_path = "/Users/riab/Documents/persona-research-internship/data/llm_edited_reviews_embedded_with_sentiment/llm_edited_reviews_embedded_with_sentiment.jsonl"
...
for i, j in orig.iterrows():
    embedding = j["full_prompt_embedding"].copy()
    embedding.append(j["full_prompt_sentiment"])
    extend_embeddings.append(embedding)

df_final = pd.DataFrame(extend_embeddings)

df_final.to_csv(
    "../data/llm_edited_reviews_embedded_with_sentiment/llm_edited_reviews_embedded_with_sentiment.csv",
    index=False,
)
